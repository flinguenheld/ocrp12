...
class TestClientsWithManagers:
    ...
    def test_manager_can_list_clients(self, api_client_manager):

        client_0 = Client.objects.create(name='Client name',
                                         salesperson=None)

        # --
        response = api_client_manager.get('/clients/')
        data = response.json()

        assert response.status_code == 200
        assert data[0]['name'] == client_0.name

    def test_manager_can_get_client_details(self, api_client_manager):

        client_0 = Client.objects.create(name='Client name',
                                         salesperson=None)

        # --
        response = api_client_manager.get(f'/clients/{client_0.pk}/')
        data = response.json()

        assert response.status_code == 200
        assert data['name'] == client_0.name

    def test_manager_can_create_a_new_client(self, api_client_manager):
        ...
        response = api_client_manager.post('/clients/', data=body)
        data = response.json()
