...
class TestContractWithSalespeople:
    ...
    def test_technical_support_can_list_contracts(self, client_technical_support):
        ...
        # --
        response = client_technical_support.get('/contracts/')
        data = response.json()
    ...
    def test_salesperson_can_get_client_details(self, client_technical_support):
    ...
    def test_technical_support_cannot_create_a_new_contract(self, client_technical_support):
        ...
        response = client_technical_support.post('/contracts/', data=body)
        data = response.json()

        assert response.status_code == 403
        assert 'Only the assigned salesperson or managers are authorized to do this request' in data['detail']

    def test_technical_support_cannot_update_a_contract(self, client_technical_support):
        ...
        response = client_technical_support.put(f'/contracts/{contract.pk}/', data=body)
        data = response.json()
    ...
    def test_salesperson_cannot_delete_a_contract(self, client_technical_support):
        ...
        # --
        response = client_technical_support.delete(f'/contracts/{contract_to_delete.pk}/')
        data = response.json()
